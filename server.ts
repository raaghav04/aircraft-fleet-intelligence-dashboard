...
async function startServer() {
  ...
  app.post("/api/insights", async (req, res) => {
    ...
    if (!ai) {
      ...
      return res.json({ insights: fallbacks, source: "domain-analytics-engine", generatedAt: new Date().toISOString() });
    }
...
- Make each insight fresh, unique, and actionable.`
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          return res.json({ insights: parsed, source: "gemini-ai", generatedAt: new Date().toISOString() });
        }
      } catch (e) {
